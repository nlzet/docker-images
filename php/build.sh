...
build () {
  ...
  echo "#########"
  echo " "
  echo docker build --build-arg PHP_EXTENSIONS=\"${PHP_EXTENSIONS}\" --build-arg FROM_IMAGE=${FROM_IMAGE} --target $2 -t ${DOCKER_TAG}-$1 -t ${DOCKER_TAG_MINOR}-$1 php/ $3
...
build "minimal" "stage0" "--pull --no-cache"
build "cli" "stage1" "--cache-from=${DOCKER_PREFIX}-minimal"
